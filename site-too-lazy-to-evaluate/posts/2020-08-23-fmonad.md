...
* index-core: カインド`k -> Type`をもつ任意の型コンストラクタ`f :: k -> Type`上の関手やモナド
  * パッケージの元ネタ論文: [Kleisli arrows of outrageous fortune(pdf)](https://personal.cis.strath.ac.uk/conor.mcbride/Kleisli.pdf)
  * モナドは`IMonad`という名前
...
|             |`Sum f`    | ✔  |     | ✔ |
|             |`Free`     |      |     | ✔ |
|`Functor f =>` |`FreeT f` |      | ✔ <sup>[1]</sup> |     |
|             |`IxState` <sup>[2]</sup> | ✔ |     |     |

<sup>[1]</sup> mmorphパッケージには無いけれど多分なれます。
また、`MMonad (FreeT f)`と`f`を固定して`m`を引数と見るのでなく、`m`を固定して`f`を引数と見るなら、[`FreeT _ m`は`FMonad`です](https://github.com/viercc/kitchen-sink-hs/blob/master/experiment/src/FreeT_FMonad.md)。
